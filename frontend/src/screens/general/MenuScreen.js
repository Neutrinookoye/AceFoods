import React , { useState , useEffect } from 'react'
import { useSelector , useDispatch } from 'react-redux'
import { Link } from 'react-router-dom'
import { Row, Col, Image, ListGroup, Card, Button , Form } from 'react-bootstrap'
import '../../styles/menuStyles.css'
import Message from '../../components/Message'
import Loader from '../../components/Loader' 
import Rating from '../../components/Rating'
import { listMenuDetails } from '../../actions/menuActions'

const MenuScreen = ({ history , match }) => {

    const dispatch = useDispatch()

    const menuDetails = useSelector(state => state.menuDetails)
    const { loading , error , menu } = menuDetails

    useEffect(() => {
        dispatch(listMenuDetails(match.params.id))
    } , [dispatch , match])

    const [qty , setQty] = useState(1) //Item quantity is a component level state

    const addToCartHandler = () => {
        history.push(`/cart/${match.params.id}?qty=${qty}`)
    }

    return (
        <>
            <Link to='/' className='btn btn-light my-4'>Go Back</Link>
            {
                loading ? (<Loader />)
                : error ? (<Message variant='danger'>{error}</Message>)
                :(
                    <Row>
                        <Col md={6}>
                            <Image className='single-menu-img' src={menu.image} alt={menu.name} fluid />
                        </Col>
                        <Col md={3}>
                            <ListGroup variant="flush">
                                <ListGroup.Item>
                                    <h3> {menu.name} </h3>
                                </ListGroup.Item>
                                <ListGroup.Item>
                                    <Rating value={menu.rating} text={`${menu.numReviews} reviews`} />
                                </ListGroup.Item>
                                <ListGroup.Item>
                                    Price: ${menu.price}
                                </ListGroup.Item>
                                <ListGroup.Item>
                                    Description: {menu.description}
                                </ListGroup.Item>
                            </ListGroup>
                        </Col>
                        <Col md={3}>
                            <Card>
                                <ListGroup variant='flush'>
                                    <ListGroup.Item>
                                        <Row>
                                            <Col>
                                                Price:
                                            </Col>
                                            <Col>
                                                {menu.price}
                                            </Col>
                                        </Row>
                                    </ListGroup.Item>
                                    <ListGroup.Item>
                                        <Row>
                                            <Col>
                                                Status:
                                            </Col>
                                            <Col>
                                                {menu.available > 0 ? 'Available' : 'Not Available'}
                                            </Col>
                                        </Row>
                                    </ListGroup.Item>
                                    {
                                        menu.available > 0 && (
                                            <ListGroup.Item>
                                                <Row>
                                                    <Col> Qty </Col>
                                                    <Col>
                                                        <Form.Control as='select' value={qty} onChange={(e) => {
                                                        setQty(e.target.value)}}>
                                                            {
                                                                [...Array(100).keys()].map( (x) => (
                                                                    <option key={x+ 1} value={x + 1}>
                                                                        {x + 1}
                                                                    </option>
                                                                ))
                                                            }
                                                        </Form.Control>
                                                    </Col>
                                                </Row>
                                            </ListGroup.Item>
                                        )
                                    }
                                    <ListGroup.Item>
                                        <Button 
                                        className='btn-block' 
                                        type='button'
                                        onClick={addToCartHandler}
                                        disabled={!menu.available}>
                                            ADD TO CART
                                        </Button>
                                    </ListGroup.Item>
                                </ListGroup>
                            </Card>
                        </Col>
                    </Row>
                )
            }
        </>
    )
}

export default MenuScreen
